...
function Dashboard() {
    ...
    useEffect(() => {
        ...
        .then(({data}) => {
            setTotalProducts(data.totalProducts.count);
            setTotalOrdersPending(data.totalOrders.pending.count);
            setTotalOrdersSuccess(data.totalOrders.success.count);
            setTotalUsers(data.totalUsers.count);
            setTotalNews(data.totalNews.count);
            setTotalMoney(data.totalMoney.total);
            setProductsMostVỉew(data.mostViewedProducts);
            setProductMostSold(data.mostSellableProducts);
            const month = data.revenue.map(item => item.day);
            const revenue = data.revenue.map(item => item.revenue);
            setRevenue({
                labels: month,
                datasets: [
                    {
            ...
            const days = data.viewsDaily.map(item => item.day);
            const viewsDaily = data.viewsDaily.map(item => item.total);
            const soldDaily = data.soldDaily.map(item => item.total);
            setChartData({
                labels: days,
                datasets: [
                    {
            ...
            const times = data.accessTimes.map(item => item.total_count)
            setChartData1({
        ...
        return (
            <div className="border-1 surface-border border-round text-center p-1">
                ...
                <div>
                    ...
                    <Tag value={`Số lượng: ${product.quantity}`} severity="success"></Tag>
    ...
    const productSoldTemplate = (product) => {
        return (
            <div className="border-1 surface-border border-round text-center p-1">
                ...
                <div>
                    <h4 className="mb-1">{product.name}</h4>
                    <Tag value={`Số lượng bán được: ${product.total_quantity_sold ?? 0}`} severity="success"></Tag>
                    <div className="mt-2 justify-content-center">
                        <Link to={`/admin/san-pham/chinh-sua/${product.id}`}><Button icon="pi pi-search" className="rounded-3" /></Link>
                    </div>
                </div>
